...
def create_combined_characteristic_playlist(df, sp, user_id, cluster_features, min_tracks=10):
    combined_tracks = []
    for cluster_label, features in cluster_features.items():
        ...
        combined_tracks.extend(track_ids)

    # Create a single combined playlist
    playlist_name = "Combined Characteristic-Based Playlist (CBF)"
    playlist_link = create_playlist_on_spotify(sp, user_id, combined_tracks, playlist_name)
    
    return {"CBF": playlist_link}
...
def get_recommendations(access_token, user_id):
    ...
    # New KMeans playlist (songs from JSON data with features similar to user's KMeans clusters)
        ...
        characteristic_playlist_links = create_combined_characteristic_playlist(df, sp, user_id, cluster_features)
...
access_token = 'BQBasCrFDfyvP9bs2SHUBggtD9dWDDqHLd5g6-kMyfFRvTn6bnJUGoyC_-ajsQTIImLBg3TGP6fuftIjJt9eZSI4CEVn9ePfQVukSF6egY22q2rsP95shjADrfP6nmb4-ADrnFhd6KvEJRRCqn4JEpl492Jv4AOmeTQbkMQj-9mMJ7goHG2_hAakomtY6CnU4FnoboG95tdjmW2IA1Op589gsAHvaXsjB1923rlwfPflwhuL0o7_VCOdw2nvkyQdql7lzQYC-j-aQ69yRfq-qt3s'
