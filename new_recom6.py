...
access_token = 'BQAkCJiIYuKuMHaAJH7eLUMRdgsGPPbJshhy5SKoJmbvoJEety7X7F9gBq4edUTfo4GEjZMaieGPSBohzJhkHI2INYId-2OSKj0DLlI-lyycmiU997_LlHoeCw6UVSoq9RF6EAoge8nKUbbWGcdMvu4cy4toEOdZ-_MrqOp0Vc0tsdSpYzLmAaKgIvUu_uynGNSxgxxpZT4EzRIjd_UO3tPnbvwCnpIX86kSgpLwxQWIBcenhkEfax8-YxUSEaC1IG2pWpSQW0Gm4kAJHTr70lmO'
...
# Function to filter songs by cluster (KMeans or DBSCAN) optimized for the Spotify recommendation system
def filter_songs_by_cluster_optimized(song_data, cluster_labels, desired_cluster):
    """
    Filter songs by a specific cluster label.
    
    Parameters:
    song_data (list): List of songs.
    cluster_labels (list): List of cluster labels for each song.
    desired_cluster (int): The cluster label to filter songs by.

    Returns:
    list: Filtered list of songs belonging to the desired cluster.
    """
    return [song for song, cluster in zip(song_data, cluster_labels) if cluster == desired_cluster]

# Function to filter songs based on SVD similarity, optimized for the Spotify recommendation system
def filter_songs_by_svd_similarity_optimized(song_data, svd_features, user_svd_features, threshold_similarity):
    """
    Filter songs based on SVD feature similarity.
    
    Parameters:
    song_data (list): List of songs.
    svd_features (array): Array of SVD features for each song.
    user_svd_features (array): SVD feature array of the user.
    threshold_similarity (float): Threshold for similarity measure.

    Returns:
    list: Filtered list of songs based on the similarity threshold.
    """
    return [song for song, features in zip(song_data, svd_features) if cosine_similarity([features], [user_svd_features])[0][0] > threshold_similarity]
